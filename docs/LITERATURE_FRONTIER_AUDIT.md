...
| Lovett–Porat, *A Space Lower Bound for Dynamic Approximate Membership Data Structures*, FOCS 2010 / SICOMP 2013 ([DOI](https://doi.org/10.1137/100806763)) | 对常数 \(\varepsilon\) 给出动态/甚至 incremental filter 超过静态的线性额外空间 | 参数与证明依赖大宇宙；不处理 \(|U|=(1+\lambda)n\)；不要求 HI | “动态性产生额外空间”已有先例；新意必须是 dense finite-universe 的 sharp rate 或 HI separation |
