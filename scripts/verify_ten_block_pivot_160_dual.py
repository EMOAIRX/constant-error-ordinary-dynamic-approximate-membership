#!/usr/bin/env python3
"""Fraction-only convex-dual certificate for the ten-block constant > 1.60.

Every comparison is exact.  Natural logarithms are enclosed using the atanh
series after exact power-of-two range reduction; floating point is used only
for readable output.
"""
...
def log_unit_interval(x: F, terms: int = 160) -> Interval:
    """Enclose ln(x) for 1 <= x <= 2 by an exact series and tail bound."""
    assert F(1) <= x <= F(2)
    z = (x - 1) / (x + 1)
    partial = F(0)
    power = z
...
def log_interval(x: F) -> Interval:
    ...
    while reduced >= 2:
        reduced /= 2
        exponent += 1
    return add(log_unit_interval(reduced), scale(F(exponent), LN2))
...
def branch_values(z: list[F]) -> list[Interval]:
    ...
    for r in range(BLOCKS - 1):
        terms = [b_value(z[i]) for i in range(r + 1)]
        terms += [phi_value(z[r], z[j]) for j in range(r + 1, BLOCKS)]
        values.append(scale(F(1, BLOCKS), sum_intervals(terms)))
...
def main() -> None:
    z = [
        decimal_fraction(x)
        for x in (
            "0.24607073074127461", "0.41943582092445958",
            "0.54874422327959660", "0.64917775981383674",
            "0.72956444610870852", "0.79540135190501848",
            "0.85029461667573258", "0.89670648012901277",
            "0.93636784338373746", "0.97051323016209312",
        )
    ]
    weights = [
        decimal_fraction(x)
        for x in (
            "0.236811761162405", "0.092787322757503",
            "0.074942840662885", "0.066524701834436",
            "0.061560128205742", "0.058483982270063",
            "0.056758934426834", "0.056276433051068",
            "0.057287271113586", "0.060711040113579",
            "0.177855584401899",
        )
    ]
    ...
    for x, derivative in zip(z, gradient):
        loss += max(abs(derivative[0]), abs(derivative[1])) * max(x, 1 - x)
    ...
    print(f"gradient_residual_bound={float(loss):.15e}")
    ...
    print("PASS: ten-block constant > 1.60")
